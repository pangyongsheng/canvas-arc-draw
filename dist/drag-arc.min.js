	...
	*/
	...
	var DragAcr = /*#__PURE__*/function () {
	  function DragAcr(param) {
	    _classCallCheck(this, DragAcr);

	    this.initParam(param);
	    this.draw(this.value);
	  }

	  _createClass(DragAcr, [{
	    key: "initParam",
	    ...
	    value: function draw(value) {
	      ...
	      fill$2(_context2 = this.ctx).call(_context2); // 文字
	    ...
	    value: function respotchangeXY(point) {
	      ...
	      var spotchangeX = function spotchangeX(i) {
	        return i + _this3.center;
	      };

	      var spotchangeY = function spotchangeY(i) {
	        return _this3.center - i;
	      };
	    ...
	    value: function setLinearGradient() {
	      ...
	      forEach$3(_context3 = this.color).call(_context3, function (e, i) {
	        if (i == 0) {
	          grad.addColorStop(0, e);
	        } else if (i == _this4.color.length - 1) {
	          grad.addColorStop(1, e);
	        } else {
	          grad.addColorStop(1 / _this4.color.length * (i + 1), e);
	        }
	      });
	    ...
	    value: function event(dom) {
	      ...
	      if (this.isMobile) {
	        ...
	        dom.addEventListener("touchstart", bind$2(_context4 = this.OnMouseDown).call(_context4, this), false);
	        dom.addEventListener("touchmove", this.throttle(bind$2(_context5 = this.OnMouseMove).call(_context5, this)), false);
	        dom.addEventListener("touchend", bind$2(_context6 = this.OnMouseUp).call(_context6, this), false);
	        return;
	      }

	      dom.addEventListener("mousedown", bind$2(_context7 = this.OnMouseDown).call(_context7, this), false);
	      dom.addEventListener("mousemove", this.throttle(bind$2(_context8 = this.OnMouseMove).call(_context8, this)), false);
	      dom.addEventListener("mouseup", bind$2(_context9 = this.OnMouseUp).call(_context9, this), false);
	    }
	  }, {
	    key: "OnMouseMove",
	    value: function OnMouseMove(evt) {
	      ...
	      var val = (deg / Math.PI - this.startDeg) / (this.endDeg - this.startDeg) * 100;
	      if (val < 0) val = 100 + val; // if(val>100 || val<0) return;

	      if (val >= 100) val = 100;
	      if (val <= 0) val = 0;
	      if (Math.abs(val - this.value) > 10) return;
	      this.animate = requestAnimationFrame(bind$2(_context10 = this.draw).call(_context10, this, val));
